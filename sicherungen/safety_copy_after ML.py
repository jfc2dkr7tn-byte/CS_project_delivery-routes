...
def show_calculation_page():
    ...
    with st.spinner("Calculating optimal route... This may take a moment for multiple stops."): # show this spinner while calculating
        try:
            ...
            # Berechnung ML-basierte Fahrzeitkorrektur
            corrected_time_min = predict_corrected_time(
                ...
                elevation_gain=100,          # Schätzung (TomTom liefert es nicht)
                traffic=1.0                  # Standardwert (kann später verbessert werden)
            )
            ...
            with tab1:
                ...
                with col3:
                    st.metric("ML-Corrected Travel Time", f"{corrected_time_min/60:.2f} hours")
